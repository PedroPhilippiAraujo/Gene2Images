...
# Sanity Check: Is expression data normalized?
```{python}
# Expression data (patients x genes) 
expr_check = expr_transposed_df.copy()

# add Claudin subtype labels 
expr_check['CLAUDIN_SUBTYPE'] = meta_df.loc[
    expr_check.index, 'CLAUDIN_SUBTYPE'
]

# Remove samples without subtype
expr_check = expr_check.dropna(subset=['CLAUDIN_SUBTYPE'])

# Subsample genes
np.random.seed(42)
n_genes = 100  # number of genes to sample
sampled_genes = np.random.choice(
    expr_check.columns[:-1],  # exclude subtype column
    size=n_genes,
    replace=False
)

expr_subset = expr_check[sampled_genes.tolist() + ['CLAUDIN_SUBTYPE']]
...
plt.title("Sanity Check: Gene Expression (z-scores) per Claudin Subtype")
plt.xlabel("Claudin Subtype")
