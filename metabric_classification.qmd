...
# PCA representation with violin plots
```{python}
...
pca_long_df = pd.DataFrame({
    "PC1": z_2d[:, 0],
    "PC2": z_2d[:, 1],
    "Claudin_Subtype": label_names
})

# Melt wide → long
pca_long_df = pca_long_df.melt(
...
plt.title("Distribution of PCA Components (PC1 & PC2) by Claudin Subtype", fontsize=14, fontweight="bold")
...
# PCA representation before dropping NA values

```{python}
expr_values = expr_transposed_df.values 
pca = PCA(n_components=2)
z_2d = pca.fit_transform(expr_values)

# Claudin Subtype Labels
labels = meta_df.loc[expr_transposed_df.index, "CLAUDIN_SUBTYPE"].map({
    "LumA": 6,
    "LumB": 5,
    "Her2": 4,
    "claudin-low": 3,
